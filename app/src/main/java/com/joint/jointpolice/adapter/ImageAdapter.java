...
public class ImageAdapter extends BaseRecycleAdapter<LocalMedia> {
    ...
    public int getResourceView(int resource) {
        return R.layout.item_locale_image;
    }

    @Override
    public void getItemView(RecycleViewHolder viewHolder, int position, LocalMedia item) {
        viewHolder.getConvertView().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                LUtils.toast("11111");
//                PictureSelector.create((Activity) mContext).openGallery(PictureMimeType.ofImage()).theme(R.style.picture_default_style);

                int themeId = R.style.picture_default_style;
                int chooseMode = PictureMimeType.ofAll();
                int maxSelectNum = 100;
